...
# SETRAF v2.0.0 - HuggingFace Spaces - Updated 2025-12-15 21:15
# Configuration TensorFlow et Matplotlib AVANT tous les imports
import os
os.environ['CUDA_VISIBLE_DEVICES'] = '-1'  # Force CPU pour TensorFlow
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'  # Réduit les logs TensorFlow
os.environ['MPLBACKEND'] = 'Agg'  # Force matplotlib backend sans display

# Configurer matplotlib avant import
import matplotlib
matplotlib.use('Agg')  # Backend sans GUI, essentiel pour Streamlit
...
# Configuration des modèles HuggingFace (téléchargement automatique)
SETRAF_BASE_PATH = os.path.dirname(os.path.abspath(__file__))
# Utiliser directement les identifiants HF pour téléchargement auto
# Phi-3-mini-4k: Modèle léger 3.8B paramètres (~3GB RAM)
PHI3_MODEL_PATH = "microsoft/Phi-3-mini-4k-instruct"  # Téléchargé depuis HF
CLIP_MODEL_PATH = "openai/clip-vit-base-patch32"  # Téléchargé depuis HF
...
# Lire les tokens depuis les variables d'environnement (configurés dans HF Spaces)
# Noms correspondent aux variables configurées: HF et TAVLY
HF_TOKEN = os.environ.get("HF", os.environ.get("HF_TOKEN", ""))
TAVILY_API_KEY = os.environ.get("TAVLY", os.environ.get("TAVILY_API_KEY", ""))
...
class ERTKnowledgeBase:
    ...
    def initialize_embeddings(self):
        ...
        try:
            ...
            # Forcer CPU et désactiver meta device
            ...
            # Utiliser l'identifiant HuggingFace pour téléchargement automatique
            embeddings_model = "sentence-transformers/all-MiniLM-L6-v2"
            
            # Télécharger depuis HuggingFace automatiquement
            st.info(f"📥 Téléchargement du modèle: {embeddings_model}")
            
            # Charger avec paramètres explicites pour éviter meta device
            self.embeddings = SentenceTransformer(
                embeddings_model,
                device='cpu',
                cache_folder=None,  # Utiliser le cache par défaut
            ...
            st.error(f"⚠️ Impossible de charger les embeddings : {str(e)}")
            import traceback
            st.error(f"Détails: {traceback.format_exc()[:500]}")
            return False
...
def explain_operation_with_llm(llm_pipeline, operation_type, operation_data, 
    ...
    Args:
        llm_pipeline: Pipeline Phi-3-mini chargé
...
def explain_with_cache(llm_pipeline, operation_type, operation_data, context=""):
    ...
    Args:
        llm_pipeline: Pipeline Phi-3-mini
...
def load_clip_model():
    ...
    try:
        ...
        # Charger CLIP depuis HuggingFace Hub (cache automatique)
        model = CLIPModel.from_pretrained(
            "openai/clip-vit-base-patch32",
            token=HF_TOKEN  # Token HuggingFace si nécessaire
        )
...
@st.cache_resource(hash_funcs={type(lambda: None): lambda _: "phi3-mini-v2"})
def load_phi3_llm(use_cpu=True, quantize=True):
    """
    Charge le modèle Phi-3-mini LLM ULTRA-OPTIMISÉ avec gestion mémoire stricte
    Modèle léger 3.8B paramètres (~3-4GB RAM)
    ...
    try:
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            PHI3_MODEL_PATH,  # Télécharger depuis HuggingFace
        ...
        st.info("📦 Chargement RAPIDE du modèle (optimisé CPU)...")
        
        # Charger SANS accelerate pour éviter conflits DynamicCache
        ...
        # Placer sur CPU explicitement
        model = model.to('cpu')
        
        # CRITIQUE: Désactiver cache dans la config
        ...
        # Pipeline OPTIMISÉ - Ne PAS spécifier device car déjà défini dans device_map
        llm_pipeline = pipeline(
            "text-generation",
            model=model,
            tokenizer=tokenizer,
            framework="pt",
            max_new_tokens=256,  # Réponses courtes et rapides
            do_sample=True,
            temperature=0.7,
            top_p=0.9,
        ...
        st.warning("💡 Le modèle Phi-3-mini nécessite ~4GB RAM. Vérifiez les ressources disponibles.")
        return None

def load_phi3_llm_basic(use_cpu=True):
    ...
    try:
        ...
        st.success("✅ LLM Phi-3-mini chargé (mode standard) !")
        return llm_pipeline
    except Exception as e:
...
def analyze_data_with_phi3(llm_pipeline, geophysical_data, progress_callback=None):
    """
    ...
    try:
        ...
        if progress_callback:
            progress_callback("🧠 Génération avec Phi-3-mini...", 0.3)
        
        # OPTIMISATION : Génération avec STREAMING token par token
        if progress_callback:
            progress_callback("💭 Génération avec streaming...", 0.5)
        
        # Utiliser le streaming pour affichage en temps réel
        try:
            from transformers import TextIteratorStreamer
            from threading import Thread
            import streamlit as st
            
            # Préparer le placeholder pour streaming
            streaming_placeholder = st.empty()
            
            # Tokenizer
            inputs = llm_pipeline.tokenizer(context, return_tensors="pt")
            device = next(llm_pipeline.model.parameters()).device
            ...
            # Lancer génération dans thread séparé
            thread = Thread(target=llm_pipeline.model.generate, kwargs=generation_kwargs)
            thread.start()
            
            # Collecter tokens sans mise à jour excessive
            response_text = ""
            token_count = 0
            update_every = 5  # Mettre à jour tous les 5 tokens pour éviter tremblements
            
            for new_text in streamer:
                response_text += new_text
                token_count += 1
                
                # Mettre à jour seulement tous les N tokens pour éviter tremblements UI
                if token_count % update_every == 0 or token_count == 1:
                    streaming_placeholder.info(f"🤖 Génération... {token_count} tokens")
            
            thread.join()
            
            # Afficher le résultat final proprement
            streaming_placeholder.success(f"✅ Génération terminée ({token_count} tokens)")
            st.markdown(f"**💬 Interprétation LLM:**\n\n{response_text}")
            
        except Exception as e:
            # Fallback sans streaming
            st.warning(f"⚠️ Streaming désactivé: {str(e)[:50]}")
            with torch.inference_mode():
                ...
                response_text = llm_pipeline.tokenizer.decode(outputs[0], skip_special_tokens=True)
                response_text = response_text.replace(context, "").strip()
        ...
        if '[/INST]' in response_text:
            generated_text = response_text.split('[/INST]')[-1].strip()
        else:
...
# Phi-3-mini-4k est assez léger pour HuggingFace Spaces (3GB RAM)
if not st.session_state.llm_loaded and not st.session_state.llm_loading_attempted:
    ...
    with st.sidebar.status("🤖 Chargement automatique du LLM Phi-3-mini...", expanded=True) as status:
        try:
            st.sidebar.write("📥 Initialisation du modèle LLM...")
            st.session_state.llm_pipeline = load_phi3_llm(use_cpu=True, quantize=True)
            ...
            st.sidebar.success("💡 Analyses IA activées (LLM Phi-3-mini)")
...
if st.session_state.llm_loaded:
    st.sidebar.success("✅ LLM Phi-3-mini actif - Analyses intelligentes activées")
...
# Initialiser avec valeur par défaut sûre
if 'fast_mode' not in st.session_state:
    st.session_state['fast_mode'] = True  # Par défaut en mode rapide

# Radio simple sans index dynamique problématique
mode_options = ["⚡ Rapide", "🧠 Complet"]
mode_selected = st.sidebar.radio(
    "Choisissez le mode :",
    options=mode_options,
    index=0,  # Toujours commencer sur "Rapide"
    help="**Mode Rapide** : Affichage instantané des résultats\n**Mode Complet** : + Analyse LLM Phi-3 (2-3 min)"
)

# Mettre à jour le state basé sur la sélection
st.session_state['fast_mode'] = (mode_selected == "⚡ Rapide")

if st.session_state['fast_mode']:
    st.sidebar.success("⚡ Mode Rapide - Résultats instantanés")
else:
    st.sidebar.info("🧠 Mode Complet - Analyse LLM disponible")
    ...
    if uploaded_file is not None:
        ...
        if not df.empty:
            ...
            if st.session_state.get('llm_loaded', False):
                ...
                # Zone de saisie (utiliser text_input car chat_input ne fonctionne pas dans les tabs)
                col_input, col_btn = st.columns([4, 1])
                with col_input:
                    user_question = st.text_input("Question", placeholder="Posez une question (scientifique, technique, interprétation...)", label_visibility="collapsed")
                with col_btn:
                    ask_button = st.button("📤 Envoyer", use_container_width=True)
...
Réponds en français, concis (100-200 mots). [/INST]"""
            ...
            ax.grid(True, alpha=0.3)
            plt.tight_layout()
            st.pyplot(fig_cluster, clear_figure=False)
            ...
            if len(survey_points) >= 2 and len(depths) >= 2:
                ...
                st.pyplot(fig_water, clear_figure=False)
...
- Profondeur : {df['depth'].abs().min():.1f} - {df['depth'].abs().max():.1f} m
            ...
            plt.tight_layout()
            st.pyplot(fig_cbar, clear_figure=False)
            plt.close()
            ...
            with st.expander("🔴 Coupe 1 - Zone d'eau de mer (0.1 - 1 Ω·m)", expanded=False):
                ...
                if seawater_mask.sum() > 0:
                    ...
                    plt.tight_layout()
                    st.pyplot(fig_sea, clear_figure=False)
...
- **Zone** : Eau océanique fortement salée
            ...
            with st.expander("🟡 Coupe 2 - Nappe d'eau salée (1 - 10 Ω·m)", expanded=False):
                ...
                if saline_mask.sum() > 0:
                    ...
                    plt.tight_layout()
                    st.pyplot(fig_saline, clear_figure=False)
...
- **Zone** : Eau saumâtre dans nappe phréatique
            ...
            with st.expander("🟢 Coupe 3 - Aquifère d'eau douce (10 - 100 Ω·m)", expanded=False):
                ...
                if fresh_mask.sum() > 0:
                    ...
                    plt.tight_layout()
                    st.pyplot(fig_fresh, clear_figure=False)
...
- **Zone** : Eau douce continentale
            ...
            with st.expander("🔵 Coupe 4 - Eau très pure / Roche sèche (> 100 Ω·m)", expanded=False):
                ...
                if pure_mask.sum() > 0:
                    ...
                    plt.tight_layout()
                    st.pyplot(fig_pure, clear_figure=False)
...
- **Zone** : Eau très pure ou formation rocheuse résistive
            ...
            with st.expander("📊 Coupe 5 - Pseudo-Section de Résistivité Apparente (Format Classique)", expanded=True):
                ...
                **Carte de pseudo-section au format géophysique standard**
                ...
                plt.tight_layout()
                st.pyplot(fig_pseudo, clear_figure=False)
                plt.close()
...
- Profondeur max: {Z_real.max():.2f} m
                    ...
                    st.warning("⚠️ LLM non chargé. Cliquez sur '🚀 Charger le LLM Phi-3-mini' dans la sidebar.")
...
La couleur d'un point sur la pseudo-section représente donc la valeur de la résistivité apparente en ce point.
    ...
    if st.session_state.get('uploaded_data') is not None:
        ...
        st.pyplot(fig_real, clear_figure=False)
...
Les zones bleues indiquent des niveaux d'eau plus bas (nappe plus proche de la surface).
        ...
        with st.expander("🌊 Coupe comparative 1 - Intrusion saline côtière avec mesures", expanded=False):
            ...
            plt.tight_layout()
            st.pyplot(fig_comp1, clear_figure=False)
        ...
        with st.expander("🏔️ Coupe comparative 2 - Aquifère multicouche avec résistivités", expanded=False):
            ...
            plt.tight_layout()
            st.pyplot(fig_comp2, clear_figure=False)
        ...
        with st.expander("📊 Pseudo-Section de Résistivité Apparente (Format Classique)", expanded=True):
            ...
            **Carte de pseudo-section au format géophysique standard**
            ...
            plt.tight_layout()
            st.pyplot(fig_pseudo_t3, clear_figure=False)
            plt.close()
    ...
    if 'uploaded_data' in st.session_state and st.session_state['uploaded_data'] is not None:
        ...
        if len(df) > 0:
            ...
            **Colormap unique basée sur les types d'eau** (Rouge: mer/salée → Jaune: salée → Vert/Bleu: douce → Bleu foncé: pure).
                    ...
                    with st.expander(f"📍 **{name}** ({rho_min}-{rho_max} Ω·m)", expanded=False):
                        ...
                        if len(df_filtered) > 3:
                            ...
                            if len(X_strat) > 3:
                                ...
                                plt.tight_layout()
                                st.pyplot(fig_strat, clear_figure=False)
                                plt.close()
                            else:
            ...
            plt.tight_layout()
            st.pyplot(fig_dist, clear_figure=False)
            plt.close()
            ...
            with st.expander("📊 Pseudo-Section de Résistivité Apparente (Format Classique)", expanded=True):
                ...
                **Carte de pseudo-section au format géophysique standard**
                ...
                if len(X_real_t4) > 3:
                    ...
                    plt.tight_layout()
                    st.pyplot(fig_pseudo_t4, clear_figure=False)
                    plt.close()
    ...
    if uploaded_freq is not None:
        ...
        if not df_pygimli.empty:
            ...
            **Classification automatique :**
            ...
            plt.tight_layout()
            st.pyplot(fig_freq_pseudo, clear_figure=False)
            plt.close()
            ...
            **Points noirs :** Mesures réelles annotées avec fréquences (MHz)
            ...
            plt.tight_layout()
            st.pyplot(fig_freq_profile, clear_figure=False)
            plt.close()
            ...
            with st.expander("📊 Coupe 1 - Classification Hydrogéologique (4 classes d'eau)", expanded=True):
                ...
                plt.tight_layout()
                st.pyplot(fig_geo1, clear_figure=False)
                plt.close()
            ...
            with st.expander("📈 Coupe 2 - Gradient Vertical de Résistivité (Interfaces géologiques)", expanded=False):
                ...
                plt.tight_layout()
                st.pyplot(fig_geo2, clear_figure=False)
                plt.close()
            ...
            with st.expander("🗺️ Coupe 3 - Modèle Lithologique Interprété (Géologie complète)", expanded=False):
                ...
                plt.tight_layout()
                st.pyplot(fig_geo3, clear_figure=False)
                plt.close()
            ...
            if st.button("🚀 Lancer l'Inversion pyGIMLi", type="primary"):
                with st.spinner("🔄 Inversion en cours avec pyGIMLi..."):
                    try:
                        ...
                        plt.tight_layout()
                        st.pyplot(fig_pygimli, clear_figure=False)
                        plt.close()
                        ...
                        with st.expander("📊 Coupe Inversée 1 - Résistivité Vraie (échelle log)", expanded=True):
                            ...
                            plt.tight_layout()
                            st.pyplot(fig_inv1, clear_figure=False)
                            plt.close()
                        ...
                        with st.expander("💧 Coupe Inversée 2 - Classification Hydrogéologique", expanded=True):
                            ...
                            plt.tight_layout()
                            st.pyplot(fig_inv2, clear_figure=False)
                            plt.close()
                        ...
                        with st.expander("📈 Coupe Inversée 3 - Gradient Horizontal (Hétérogénéités)", expanded=False):
                            ...
                            plt.tight_layout()
                            st.pyplot(fig_inv3, clear_figure=False)
                            plt.close()
                        ...
                        with st.expander("🗺️ Coupe Inversée 4 - Modèle Lithologique Complet", expanded=False):
                            ...
                            plt.tight_layout()
                            st.pyplot(fig_inv4, clear_figure=False)
                            plt.close()
                        ...
                        plt.tight_layout()
                        st.pyplot(fig_classif, clear_figure=False)
        ...
        if st.button("🚀 Extraire Spectres", key="extract_spectra"):
            with st.spinner("🔄 Extraction des spectres en cours..."):
                try:
                    ...
                    plt.tight_layout()
                    st.pyplot(fig_spectra, clear_figure=False)
                ...
                if st.button("🚀 Appliquer Imputation", key="apply_imputation"):
                    with st.spinner(f"🔄 Imputation {imputation_method} en cours..."):
                        try:
                            ...
                            plt.tight_layout()
                            st.pyplot(fig_impute, clear_figure=False)
            ...
            if st.button("🚀 Modélisation Forward", key="forward_modeling"):
                with st.spinner("🔄 Modélisation forward en cours..."):
                    try:
                        ...
                        plt.tight_layout()
                        st.pyplot(fig_forward, clear_figure=False)
...
- Plage mesures bruitées: {measurements_noisy.min():.3f} à {measurements_noisy.max():.3f}
                            ...
                            st.warning("⚠️ LLM non chargé. Cliquez sur '🚀 Charger le LLM Phi-3-mini' dans la sidebar pour des explications intelligentes.")
            ...
            if st.button("🚀 Reconstruction 3D", key="reconstruct_3d"):
                with st.spinner("🔄 Reconstruction 3D en cours..."):
                    try:
                        ...
                        plt.tight_layout()
                        st.pyplot(fig_reconstruct, clear_figure=False)
                            ...
                            **Transformez vos données 3D en visualisations géologiques professionnelles.**
                            ...
                            if st.button("🚀 Générer Images Réalistes des Coupes", key="generate_realistic_3d"):
                                ...
                                if generated_img_3d is not None:
                                    ...
                                    st.pyplot(fig_comp_3d, clear_figure=False)
            ...
            if st.button("🚀 Détecter Trajectoires", key="detect_trajectories"):
                with st.spinner("🔄 Détection RANSAC en cours..."):
                    try:
                        ...
                        plt.tight_layout()
                        st.pyplot(fig_trajectories, clear_figure=False)
                            ...
                            with st.spinner("🎨 Génération de la coupe géologique avec trajectoires... (30s-2min)"):
                                try:
                                    ...
                                    if traj_generated_img is not None:
                                        ...
                                        st.pyplot(fig_traj_comparison, clear_figure=False)
...
- Résistivité moyenne: {rho_3d.mean():.2f} Ω·m
        ...
        if ('spectra' in st.session_state and 'rho_imputed' in st.session_state and 
            ...
            **✅ RÉSULTAT** : Sous-sol visualisé = Sous-sol réel (pas une approximation !)
            ...
            # ANALYSE RAPIDE (sans LLM)
            st.markdown("#### 📊 Analyse Rapide (Instantanée)")
            ...
            if spectra is not None and rho_reconstructed is not None:
                rho_mean = float(rho_reconstructed.mean())
                rho_min = float(rho_reconstructed.min())
                rho_max = float(rho_reconstructed.max())
                
                # Interprétation géologique rapide
                if rho_mean < 50:
                    geo_type = "🌊 **Argiles/Sédiments saturés**"
                    interpretation = "Zone à forte teneur en eau, possiblement argileuse ou sédimentaire saturée."
                elif rho_mean < 200:
                    geo_type = "🏔️ **Roches fracturées/altérées**"
                    interpretation = "Substrat rocheux fracturé ou altéré avec circulation d'eau modérée."
                elif rho_mean < 1000:
                    geo_type = "⛰️ **Roches compactes/sèches**"
                    interpretation = "Substrat rocheux compact avec faible teneur en eau."
                else:
                    geo_type = "💎 **Roches très résistives**"
                    interpretation = "Substrat très résistif (granite, quartzite) ou cavités d'air."
                
                st.info(f"""
                **Type géologique identifié:** {geo_type}
                
                **Résistivité moyenne:** {rho_mean:.1f} Ω·m (min: {rho_min:.1f}, max: {rho_max:.1f})
                
                **Interprétation:** {interpretation}
                
                **Statistiques:**
                - Nombre de mesures: {len(spectra)}
                - Structures détectées (RANSAC): {len(trajectories)}
                - Modèle 3D: {rho_3d.shape if rho_3d is not None else 'N/A'}
                """)
            
            # ANALYSE LLM (selon le mode choisi)
            st.markdown("---")
            st.markdown("#### 🤖 Analyse LLM Avancée")
            
            # Vérifier si mode rapide est activé
            if st.session_state.get('fast_mode', False):
                st.warning("⚡ **Mode Rapide activé** - L'analyse LLM est désactivée pour des résultats instantanés.")
                st.info("💡 Basculez vers le **Mode Complet** dans la sidebar pour activer l'analyse intelligente avec Phi-3.")
            else:
                st.info("ℹ️ L'analyse LLM est disponible en Mode Complet. Génération en 2-3 min (optimisée).")
                
                if st.checkbox("🤖 Activer l'analyse LLM Phi-3", value=False, key="enable_llm_final"):
                    st.info("⏳ Chargement optimisé du LLM Phi-3-mini...")
                    
                    with st.spinner("🤖 Chargement Phi-3-mini..."):
                        llm_pipeline = load_phi3_llm(use_cpu=True)
                    
                    if llm_pipeline is not None:
                        st.success("✅ Phi-3-mini chargé ! Génération optimisée en cours...")
                    ...
                    if st.button("🧠 Lancer l'analyse LLM complète", type="primary", key="llm_analysis_complete_btn"):
                        ...
                        interpretation, recommendations, llm_prompt = analyze_data_with_phi3(
                ...
                with st.spinner("🗺️ Génération des coupes géologiques réelles... (10-15s)"):
                    try:
                        ...
                        if spectra is not None and positions is not None:
                            ...
                            st.pyplot(fig_coupe1, clear_figure=False)
                        ...
                        if rho_3d is not None:
                            ...
                            st.pyplot(fig_coupe2, clear_figure=False)
...
if __name__ == "__main__":
    try:
        pass  # set_page_config déjà appelé en ligne 4051
    except Exception as e:
        import traceback
        st.error(f"❌ Erreur critique au démarrage: {str(e)}")
...
try:
    ...
    st.sidebar.error(f"⚠️ Erreur sidebar: {str(e)[:200]}")