...
class AuthManager:
    ...
    def _get_backend_url(self):
        ...
        # En production, utiliser Render en priorité
        if USE_PRODUCTION:
            try:
                response = requests.get(f"{PRODUCTION_BACKEND.replace('/api', '')}/api/health", timeout=3)
                if response.status_code == 200:
                    return PRODUCTION_BACKEND
            except:
                st.warning("⚠️ Backend Render inaccessible, tentative en local...")
        
        # Fallback sur backend local si production échoue
        port = os.getenv('PORT', '5000')
        ips_to_try = [
            "localhost",       # Localhost d'abord
            "172.20.16.1",     # IP Windows host depuis WSL
